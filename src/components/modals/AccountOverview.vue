<template>
	<div class="w-full bg-[#000] text-left p-[20px] mb-[20px]">
		<h2 class="text-[#868C9D] text-[14px] mb-[10px] uppercase">
			Connected Wallet
		</h2>
		<h3 class="text-white text-[14px]">
			{{ walletAddress }}
		</h3>
		<div class="mt-[20px] flex flex-col gap-[20px]">
			<!-- snowtrace  -->
			<a
				class="util uppercase text-[#868C9D] hover:text-white text-[14px] flex gap-[4px] items-center"
			...
			<!-- copy address  -->
			<button
				class="util uppercase text-[14px] flex gap-[4px] items-center"
				:class="[
					store.copiedStatus
						? 'text-green-700'
						: 'text-[#868C9D] hover:text-white',
				]"
					...
					>
						<g clip-path="url(#clip0_364_18870)">
							<path
								...
								:fill="[
									store.copiedStatus ? 'rgb(21 128 61)' : '#868C9D',
								]"
							/>
							<path
...
function copyToClipboard(str) {
	...
	store.copiedStatus = true;
	setTimeout(() => (store.copiedStatus = false), 3000);
}
</script>
