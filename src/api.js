...
async function createPortfolioThenDeposit(){
  ...
  var status = await createPortfolio(newWalletObject.walletName, newWalletObject.fundFee);
  if (!status.success){
  ...
  if (status.prosperoWalletAddressCreated==null){
    console.log("****** prosperoWalletAddressCreated is null - problem ***** ")
  }else{
    console.log("prosperoWalletAddressCreated is NOT null, continuing... walletAddress:"+status.prosperoWalletAddressCreated)
...
function getValuesOverTimeHelper(dataOverTimeObj, prosperoWalletAddress){
  ...
  for (var key in dataOverTimeObj) {
    ...
    if (prosperoWalletAddress==thisAdd){
      var dataOverTimeObjCopy = JSON.parse(JSON.stringify(dataOverTimeObj[key]))
...
async function getValuesOverTimeForLinechartLeaderboardAddress(prosperoWalletAddress, numberOfDays){
  //console.log("getValuesOverTimeForLeaderboardAddress")
  var returned = getValuesOverTimeForLineChartHelper(leaderBoardDataOverTime, prosperoWalletAddress, numberOfDays)
  //console.log("ret:"+JSON.stringify(returned,null,2))
  return returned;
}
async function getValuesOverTimeForLinechartMyPortfolioAddress(prosperoWalletAddress, numberOfDays){
  //console.log("getValuesOverTimeForLinechartMyPortfolioAddress")
  var returned = getValuesOverTimeForLineChartHelper(myPortfoliosDataOverTime, prosperoWalletAddress, numberOfDays)
  //console.log("ret:"+JSON.stringify(returned,null,2))
  return returned;
}
function getValuesOverTimeForLineChartHelper(dataOverTimeObj, prosperoWalletAddress, numberOfDays){
...
async function initNewEventListener(){
  ...
  if (prosperoFactoryEventsInstance==null){
    ...
    prosperoFactoryEventsInstance.on("LatestBalancesFactory", async (tokens, users, balances, percentageOwnerships, usdInvested, usersValues, addressVars, intVars, walletName, profilePictureUrl, event) => {
      ...
      if (eventBlocksAlreadyHandledLatestBalances.includes(event.blockNumber)){
        return;
      }
      //var blocksAlreadyHadled = eventBlocksAlreadyHandledProsperoFactoryWallet
      eventBlocksAlreadyHandledLatestBalances.push(event.blockNumber+"")
    ...
    prosperoFactoryEventsInstance.on("FinishedMethodFactory", async ( methodType, eoaAddressMsgSender, prosperoWalletAddress, event) => {

      console.log("got FinishedMethodFactory"
      +" "+methodType
      +" "+eoaAddressMsgSender
      ...
      if (eventBlocksAlreadyHandledFinishedMethod.includes(event.blockNumber)){
        return;
      }
      eventBlocksAlreadyHandledFinishedMethod.push(event.blockNumber+"")
      eoaAddressMsgSender=eoaAddressMsgSender.toLowerCase();
      console.log("walletWaitingForEOA:"+walletWaitingForEOA)
      console.log("eoaAddressMsgSender:"+eoaAddressMsgSender)
 ...
 //if (true){
    ...
    for (k=0;k<tokens.length;k++){
      ...
      if (historicalPricesObject.hasOwnProperty(addLowerCase)){
        ...
        if (thisAddObj.hasOwnProperty(justDateTheGraph)){ 
          ...
          console.log("****** NEED TO REDEPLOY THE GRAPH ****** error - NO DATE getHistoricalPricesUpdateChartsData:"+justDateTheGraph+" thisAddObj:"+addLowerCase)
          console.log("thisAddObj:"+JSON.stringify(thisAddObj,null,2))
        }
      }else{
        console.log("****** NEED TO REDEPLOY THE GRAPH ****** error - NO ADDRESS getHistoricalPricesUpdateChartsData:"+justDateTheGraph+" thisAddObj:"+addLowerCase)
        console.log("thisAddObj:"+JSON.stringify(thisAddObj,null,2))
      }
    }
  ...
  for (var k=0;k<dates.length;k++){
    ...
    for (var i =0;i<lbData.length;i++){
      ...
      await updateHistoryChartsDataObject(thisProsperoWalletAddressLB, leaderBoardDataOverTime, thisDate, thisLeaderEOA)
    }
  }
  //Have to do this
  lastUsdInvested=0;
  ...
  for (var k=0;k<dates.length;k++){
    ...
    for (var i=0;i<myWalletAddresses.length;i++){
      ...
      thisProsperoWalletAddressMyPorts=thisProsperoWalletAddressMyPorts.toLowerCase();
      await updateHistoryChartsDataObject(thisProsperoWalletAddressMyPorts, myPortfoliosDataOverTime, thisDate, EOAAddress)
    }
  }
...
async function updateHistoryChartsDataObject(walletAddress, dataForHistoryChart, thisDate, userAddress){
  ...
  for (var p =0;p<graphData.length;p++){
    ...
    if (!foundIt){
      ...
      if ((thisProsperoWalletAddress == walletAddress) && (thisGraphData["date"] == thisDate)){
        ...
        if (indexOfUser==-1){
          ...
          dataForHistoryChart[walletAddress].push({date:thisDate, profit: 0, value:0, usdInvested:0})
          lastUsdInvested = 0;
        ...
        var profit = thisGraphData["usersValues"][indexOfUser] - thisGraphData["usdInvested"][indexOfUser]
        foundIt = true;
        ...
        dataForHistoryChart[walletAddress].push({date:thisDate, profit: profit, value:thisGraphData["usersValues"][indexOfUser], usdInvested:thisGraphData["usdInvested"][indexOfUser]})
        break;
      }
    }
  }
...
async function deposit(){
  ...
  try{
    var valueOfUsersPortfolioBefore = await getValueOfUsersPortfolio(selectedProsperoWalletAddress, EOAAddress, false)
    var status = await depositContract(tokens, amounts, methodType, avaxValue, selectedProsperoWalletAddress);
    if (!status.success){
      return status
    }
...
async function initLeaderBoardTableObject(){
  try{
    ...
    for (var i =0;i<leaderBoardDataHere.length;i++){
      ...
      var prosperoWalletAddress =leaderBoardDataHere[i].prosperoWalletAddress;
      prosperoWalletAddress=prosperoWalletAddress.toLowerCase();
...
async function getMyPortfoliosDataForTable(){
  //right here 
  console.log("getMyPortfoliosDataForTabl - myPortfolioDataForTable:"+JSON.stringify(myPortfolioDataForTable,null,2));
...
export {
  ...
  createPortfolio,
  getBalancesInEoa,
  deposit, updateAmount, rebalance, withdraw, getGraphData, getHistoricalPricesUpdateChartsData,
